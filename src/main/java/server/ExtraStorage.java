...
Credits to PumpkinPieArchitect, Seeker1437 for the release on the MapleDev discord

Extra storage is utilized by the NPC Cody and also by using the command "extrastorage open" or "extrastorage store"

This was originally for ores only, but i have repurposed it to use scrolls and skill books as well. This might not
be able to store all of them but it should be more than enough. If not fuck you and make another table.
...
public class ExtraStorage {
    ...
    private ExtraStorage(int id, int slots, int meso) {
        this.id = id;
    ...
    private static ExtraStorage create(int id, int world) throws SQLException {
        try (Connection con = DatabaseConnection.getConnection();
             PreparedStatement ps = con.prepareStatement("INSERT INTO ExtraStorages (accountid, world, slots, meso) VALUES (?, ?, 255, 0)")) {
            ps.setInt(1, id);
            ps.setInt(2, world);
            ps.executeUpdate();
        }
    ...
    public static ExtraStorage loadOrCreateFromDB(int id, int world) {
        ExtraStorage ret;
        try (Connection con = DatabaseConnection.getConnection();
             PreparedStatement ps = con.prepareStatement("SELECT storageid, slots, meso FROM ExtraStorages WHERE accountid = ? AND world = ?")) {
            ps.setInt(1, id);
            ps.setInt(2, world);

            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ret = new ExtraStorage(rs.getInt("storageid"), rs.getInt("slots"), rs.getInt("meso"));
                    for (Pair<Item, InventoryType> item : ItemFactory.EXTRA.loadItems(ret.id, false)) {
    ...
    public void saveToDB(Connection con) {
        try {
            try (PreparedStatement ps = con.prepareStatement("UPDATE ExtraStorages SET slots = ?, meso = ? WHERE storageid = ?")) {
            ...
            ItemFactory.EXTRA.saveItems(itemsWithType, id, con);
    ...
    public void sendStorage(Client c, int npcId) {
        if (c.getPlayer().getLevel() < 50) {
            c.getPlayer().dropMessage(1, "You may only use the extra storage once you have reached level 50.");
